...
`;
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={isCustomer ? "Store Receipt Preview" : "Thermal Receipt Preview"}
      subtitle={isCustomer ? `Bill #${bill.bill_number}` : `Bill #${bill.bill_number} • ${connectedDeviceName}`}
      maxWidth="md"
    >
      <div className="space-y-3.5">
        {/* Direct Thermal Machine Banner & Connection (Admin Only) */}
        {!isCustomer && (
        ...
        {/* Collapsible Direct Print Configuration Tip (Admin Only) */}
        {!isCustomer && (
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2.5 pt-2 border-t border-slate-100 print:hidden">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <button
              onClick={onClose}
              className="flex-1 sm:flex-none px-5 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold transition-colors cursor-pointer"
            >
              Close
            </button>
            ...
            {!isCustomer && (
              <button
                ...
                className="flex-1 sm:flex-none px-5 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white text-xs font-extrabold flex items-center justify-center gap-2 shadow-lg shadow-brand-600/20 transition-all cursor-pointer"
              >
